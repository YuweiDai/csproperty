...
.ForMember(dest => dest.PropertyType, mo => mo.MapFrom(src => src.PropertyType.ToDescription()));
    ...
    .ForMember(dest => dest.State, mo => mo.MapFrom(src => src.State.ToDescription()));
            ...
            //资产巡查
            Mapper.CreateMap<PropertyPatrolModel, PropertyPatrol>()
            .ForMember(dest => dest.PatrolPictures, mo => mo.Ignore())
            .ForMember(dest => dest.PatrolDate, mo => mo.MapFrom(src => Convert.ToDateTime(src.PatrolDate)))
                .ForMember(dest => dest.People, mo => mo.Ignore())
