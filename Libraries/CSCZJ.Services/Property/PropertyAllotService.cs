...
namespace CSCZJ.Services.Property
{
    public class PropertyAllotService : IPropertyAllotService
    {
        ...
        public virtual IPagedList<CSCZJ.Core.Domain.Properties.PropertyAllot> GetAllAllotRecords(IList<int> governmentIds, string checkState = "unchecked", string search = "", int pageIndex = 0, int pageSize = int.MaxValue, params PropertySortCondition[] sortConditions)
        {
            ...
            Expression<Func<CSCZJ.Core.Domain.Properties.PropertyAllot, bool>> expression = p => !p.Deleted;
            ...
            var propertiesAllotRecords = new PagedList<CSCZJ.Core.Domain.Properties.PropertyAllot>(query, pageIndex, pageSize);
