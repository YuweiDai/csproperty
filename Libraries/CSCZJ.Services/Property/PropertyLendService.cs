   ...
   public class PropertyLendService : IPropertyLendService
    {
        ...
        public virtual IPagedList<CSCZJ.Core.Domain.Properties.PropertyLend> GetAllLendRecords(IList<int> governmentIds, string checkState = "unchecked", string search = "", int pageIndex = 0, int pageSize = int.MaxValue, params PropertySortCondition[] sortConditions)
        {
            ...
            Expression<Func<CSCZJ.Core.Domain.Properties.PropertyLend, bool>> expression = p => !p.Deleted;
            ...
            var propertiesLendRecords = new PagedList<CSCZJ.Core.Domain.Properties.PropertyLend>(query, pageIndex, pageSize);
