...
namespace CSCZJ.Services.Property
{
    public class PropertyOffService:IPropertyOffService
    {
        ...
        public IPagedList<CSCZJ.Core.Domain.Properties.PropertyOff> GetAllOffRecords(IList<int> governmentIds, string checkState = "unchecked", string search = "", int pageIndex = 0,
            int pageSize = int.MaxValue, params PropertySortCondition[] sortConditions)
        {
            ...
            Expression<Func<CSCZJ.Core.Domain.Properties.PropertyOff, bool>> expression = p => !p.Deleted;
            ...
            var propertiesOffRecords = new PagedList<CSCZJ.Core.Domain.Properties.PropertyOff>(query, pageIndex, pageSize);
