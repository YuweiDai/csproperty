...
namespace CSCZJ.Core.Infrastructure
{
    ...
    public class EngineContext
    {
        ...
        protected static IEngine CreateEngineInstance(CSCZJConfig config)
        {
            if (config != null && !string.IsNullOrEmpty(config.EngineType))
            {
                ...
                if (!typeof(IEngine).IsAssignableFrom(engineType))
                    throw new ConfigurationErrorsException("The type '" + engineType + "' doesn't implement 'CSCZJ.Core.Infrastructure.IEngine' and cannot be configured in /configuration/QMTech/engine[@engineType] for that purpose.");
        ...
        public static IEngine Initialize(bool forceRecreate, HttpConfiguration httpConfig)
        {
            ...
            {
                var config = ConfigurationManager.GetSection("CSCZJConfig") as CSCZJConfig;
